...
def get_items_from_folder(gis, folder, item_types=None) -> list: # folder=None returns the root folder
...
def process_super_scene_service_missing_root_node(super_item): # needs work
    if "store" in super_item.constructed_obj.properties.keys():
        super_item.problems.append("This scene service appears to be missing a root node. It will not be usable in ArcGIS Online or Runtime.")
    return super_item
...
def process_super_webmap_item(webmap_item_):
    ...
    if len(webmap_item_.layers) == 0:
        webmap_item_.problems.append(f"This webmap has no layers! What a shameful excuse of a map")
...
def process_super_layer(super_layer):
    ...
    super_layer_tests = [check_super_feature_layer_for_http, check_super_feature_layer_for_production_resources, check_super_feature_layer_version, check_super_feature_layer_has_no_features]
...
def check_super_dashboard_is_old(super_dashboard_item):
    ...
    if dashboard_json['version'] < 24:
        ...
        super_dashboard_item.problems.append("This Dashboard version is a middleware beta!")
...
def check_super_feature_layer_version(super_layer):
    ...
    if arcgis_server_version <= 9.3:
        super_layer.problems.append(f"feature layer is pretty old. The ArcGIS server version is {arcgis_server_version}")
...
def check_super_feature_layer_for_production_resources(super_layer):
    ...
    if super_layer.SERVICE_RE_PROD.match(constructed_layer.url):
        super_layer.problems.append(f"This Feature Layer uses production resources {constructed_layer.url}")
...
def check_super_webmap_version(webmap_item_):
    ...
    if int(webmap_spec[0]) < 2:
        webmap_item_.problems.append(f"This webmap is outdated. It's webmap spec is {webmap_spec}")
...
def check_super_max_record_count(super_layer):
    ...
    if "maxRecordCount" not in constructed_layer.properties:
        super_layer.problems.append("This layer does not advertize its max record count!")
        return super_layer

    if constructed_layer.properties.maxRecordCount < total_features:
        super_layer.problems.append("The max record count is less than the total feature count")
...
def check_super_feature_layer_has_no_features(super_feature_layer): # Did it this way because this is also checking to see if the layer can query
