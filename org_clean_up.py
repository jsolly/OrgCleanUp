...
#@async_process_item
def process_item(index, item) -> SuperItem:
...
def get_items_from_folders(gis, folders:list, item_types=None) -> list: # folder=None returns the root folder
    all_items = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("-target_org", dest="target_org", help="The organization to tag items in", required=True)
    ...
    SUPER_ITEM.GIS_OBJ = gis.GIS(url=args.target_org, username=args.username, password=args.password, verify_cert=False)
